...
export default function AuthorForm({ initialData }: AuthorFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = adminAuthorSchema.safeParse(formData);
    ...
    try {
      const url = initialData?.id ? `/api/admin/authors?id=${initialData.id}` : '/api/admin/authors';
    ...
    <form onSubmit={handleSubmit} className="p-6 rounded-3xl bg-theme-card border border-theme glass-card space-y-5 text-theme-text max-w-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-theme-heading">Author Name *</label>
          <input
            type="text"
            value={formData.name}
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-bold text-theme-heading">Avatar Image URL</label>
        <input
          type="url"
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-bold text-theme-heading">Author Biography</label>
        <textarea
          value={formData.bio}
          onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
          rows={4}
          className="w-full px-4 py-2.5 rounded-2xl bg-theme-surface border border-theme/60 text-xs font-semibold text-theme-heading placeholder:text-theme-muted focus:outline-none focus:border-blue-500"
          placeholder="Short author biography and accomplishments..."
        />
      </div>
      ...
      <div className="flex justify-end pt-2">
        ...
        >
          ...
          <span>{isSubmitting ? 'Saving...' : initialData?.id ? 'Update Author' : 'Create Author'}</span>
        </button>
      </div>
    </form>
  );
